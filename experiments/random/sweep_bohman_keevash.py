...
===========================================
Run the Bohman–Keevash K4-free process across a range of N, persist the
best graph per N to graphs/bohman_keevash.json, write a CSV summary, and
emit log-log scaling plots into docs/images/.

This is the *authoritative* BK sweep: it uses the same proposer as
experiments/random/bohman_keevash.py (uniform-add to saturation, built on
EdgeFlipWalk) so that "the BK process" is defined in exactly one place.

Usage
-----
    python experiments/random/sweep_bohman_keevash.py \
...
from bohman_keevash import propose_adds_only  # type: ignore
...
def run_sweep(ns: list[int], trials: int, seed: int):
    rows: list[dict] = []          # all per-trial records (for scatter plots)
    bests: list[tuple] = []        # (n, best_result, search) for save()
    summary: list[dict] = []       # one row per N for the CSV
...
def make_plots(rows: list[dict], summary: list[dict], out_dir: str) -> dict:
    ...
    Older Bohman 2009 used log^{8/5} on α; that overshoots the empirical curve
    by ~2× at N=100 because Wolfovitz tightened the polylog by (log N)^{7/5}.
    """
    ...
    s_e, b_e = loglog_fit(ns, medm)
    s_a, b_a = loglog_fit(ns, meda)
    s_d, b_d = loglog_fit(ns, medd)
    s_c, b_c = loglog_fit(ns, bestc)

    # All-trials scatter arrays (may be empty if rendering from CSV only).
    ...
    # 1. Edges  |E| ~ N^{8/5} log^{1/5} N (Wolfovitz)
    ax = axs[0, 0]
    if all_n.size:
    ...
    # 2. alpha  α ~ N^{3/5} log^{1/5} N (Wolfovitz)
    ax = axs[0, 1]
    if all_n.size:
    ...
    ax.loglog(ns, th_a, "r--", lw=1.5,
              label=r"Wolfovitz $N^{3/5}\log^{1/5}\!N$")
    ...
    # 3. d_max  Δ ≈ Θ(N^{3/5} log^{1/5} N), tracks 2|E|/N
    ax = axs[1, 0]
    if all_n.size:
    ...
    # 4. c_log  c_log ≈ N^{1/5} log^{1/5} / log = N^{1/5} / log^{4/5}
    ax = axs[1, 1]
    if all_n.size:
    ...
    # Wolfovitz-derived: c_log ~ α·d/N·log d → N^{1/5} log^{2/5} / log N = N^{1/5}/log^{3/5} N.
    # (Two log^{1/5} factors from α and Δ; one log N from the denominator's ln d_max.)
...
def main() -> int:
    ...
    ap.add_argument("--no-save-graphs", action="store_true",
                    help="skip overwriting graphs/bohman_keevash.json")
