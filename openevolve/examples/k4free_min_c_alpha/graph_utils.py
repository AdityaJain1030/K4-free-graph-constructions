"""Scoring helpers — vendored subset of utils/graph_props.py."""
...
def has_independent_set(adj: np.ndarray, k: int) -> bool:
    """Fast "does the graph have an independent set of size >= k?" check.
    Exits early — much cheaper than full alpha when we only care about a
    threshold (k = alpha_target + 1)."""
    n = adj.shape[0]
    if k <= 0:
        return True
    if k > n:
    ...
    def branch(candidates: int, current_size: int):
        if found[0]:
            return
        if current_size >= k:
            found[0] = True
            return
        if current_size + popcount(candidates) < k:
            return
        if candidates == 0:
            return
        v = (candidates & -candidates).bit_length() - 1
        branch(candidates & ~nbr[v] & ~(1 << v), current_size + 1)
        if not found[0]:
            branch(candidates & ~(1 << v), current_size)
...
def c_log_value(alpha: int, n: int, d_max: int) -> float | None:
    """alpha * d_max / (n * ln(d_max)). None if d_max <= 1."""
    if d_max <= 1:
        return None
