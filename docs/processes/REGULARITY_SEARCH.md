...
> **MOVED (twice).** The original `RegularitySearch` class was retired
> in favour of a `RandomWalkSearch` policy; `RandomWalkSearch` was
> then retired in favour of `EdgeFlipWalk`. The current live
> equivalent is
> [`experiments/greedy/regularity.py`](../../experiments/greedy/regularity.py)
> (re-ported on top of `EdgeFlipWalk`, 2026-04-27) — score = `−(d_u + d_v)`,
> β=∞ (greedy argmax), run to cap-saturation, same cap sweep.
> [`experiments/random/add_edges_weighted.py --weight d_min`](../../experiments/random/add_edges_weighted.py)
> is the same scorer at finite β. Notes below describe the algorithm
> for reference.
