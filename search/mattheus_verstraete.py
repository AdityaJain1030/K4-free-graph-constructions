...
def _smallest_qnr(q: int) -> int:
    """Smallest quadratic non-residue in F_q for odd prime q."""
    for c in range(2, q):
        if pow(c, (q - 1) // 2, q) == q - 1:
            return c
    raise ValueError(f"no QNR for q={q}")  # unreachable for q >= 3 prime
...
class MattheusVerstraeteSearch(Search):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        q = _q_from_n(n)
        if q is None:
            valid = {qq: _n_for_q(qq) for qq in _VALID_PRIME_QS}
            raise ValueError(
                f"n={n} not supported; MV-Hq* requires n = q²(q²−q+1) for "
                f"prime q in {_VALID_PRIME_QS}. Valid n values: {valid}."
            )
        super().__init__(
            n,
            top_k=top_k,
