#!/usr/bin/env python3
"""
scripts/run_cayley_tabu_gap.py
==============================
Drive `CayleyTabuGapSearch` (GAP-backed Cayley tabu) across a range
of N and persist results under source='cayley_tabu_gap'.

Identical interface to `scripts/run_cayley_tabu.py` — the only
difference is the search class. Defaults are tuned for the first
local pass (N=10..30); bump them when moving to the cluster.

Run::

    micromamba run -n k4free python scripts/run_cayley_tabu_gap.py \\
        --n-lo 10 --n-hi 30 --n-iters 300 --n-restarts 3 \\
        --time-limit 180 --top-k 3
"""
...
from search import AggregateLogger, CayleyTabuGapSearch
...
def _best_c_per_n_from_db(store: GraphStore, source: str = SOURCE) -> dict[int, float]:
    """Best c_log per N in graph_db under the given source."""
    best: dict[int, float] = {}
...
def main() -> int:
    ...
    ap.add_argument("--better-only", action="store_true",
                    help="Skip N's where the db already has a c_log ≤ what this run "
                         f"finds. Reads existing bests from graph_db (source='{SOURCE}').")
    ...
    with AggregateLogger(name="cayley_tabu_gap_sweep") as agg:
        for n in n_values:
            t0 = time.monotonic()
            ...
            if best:
                print(
                    ...
                    f"group={best.metadata.get('group') or '—':<24}  "
                    f"({dt:.1f}s) {note}",
                    flush=True,
                )
            else:
